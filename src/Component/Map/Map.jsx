...
const Map = () => {
  const { map, setMap, crashes, posts } = useContext(MapContext);
  ...
  useEffect(() => {
    ...
    if (crashes && posts) {
      ...
      m.on("load", async () => {
        ...
        const crash_features = crashes.filter(
          (d) =>
            +d.properties.hour > 7 &&
        ...
        m.on("click", "guards", (e) => {
          ...
          let content = `<div class="content">
                          <h4>Post ${properties["POST #"]} is ${properties["LAST NAME"] === "VACANT"
              ? '<span class="vacant">Vacant</span>'
              : "Staffed"
            }</h4>
                          <span style="font-size:24px;font-weight:bold">${crashes.length === 0
              ? 0
              : data[0].count + data[1].count
            }</span> 
                          <br/> crashes occurred near this post on the ${properties["STREET NAME 1"]
